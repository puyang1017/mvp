...
public class PyUINotchHelper {
    ...
    public static boolean has3rdNotch(Context context){
        if (PyUIDeviceHelper.isHuawei()) {
            return hasNotchInHuawei(context);
        } else if (PyUIDeviceHelper.isVivo()) {
            return hasNotchInVivo(context);
        } else if (PyUIDeviceHelper.isOppo()) {
            return hasNotchInOppo(context);
        } else if (PyUIDeviceHelper.isXiaomi()) {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (PyUIDeviceHelper.isHuawei()) {
            boolean isHuaweiNotchSetToShow = PyUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        Rect rect = new Rect();
        if (PyUIDeviceHelper.isVivo()) {
            // TODO vivo 显示与亮度-第三方应用显示比例
            rect.top = getNotchHeightInVivo(context);
            rect.bottom = 0;
        } else if (PyUIDeviceHelper.isOppo()) {
            // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
            rect.top = PyUIStatusBarHelper.getStatusbarHeight(context);
            rect.bottom = 0;
        } else if (PyUIDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (PyUIDeviceHelper.isVivo()) {
            ...
            rect.left = PyUIStatusBarHelper.getStatusbarHeight(context);
            rect.right = 0;
        } else if (PyUIDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (PyUIDeviceHelper.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = PyUIStatusBarHelper.getStatusbarHeight(context);
        } else if (PyUIDeviceHelper.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (PyUIDeviceHelper.isVivo()) {
            ...
            rect.right = PyUIStatusBarHelper.getStatusbarHeight(context);
            rect.left = 0;
        } else if (PyUIDeviceHelper.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return PyUIDisplayHelper.getStatusBarHeight(context);
    }

    public static int getNotchWidthInVivo(Context context){
        return PyUIDisplayHelper.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return PyUIDisplayHelper.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
       return (PyUIDeviceHelper.isXiaomi() || PyUIDeviceHelper.isVivo()) && PyUINotchHelper.hasNotch(view);
    }
