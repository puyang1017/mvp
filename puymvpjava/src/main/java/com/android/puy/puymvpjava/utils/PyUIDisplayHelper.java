 ...
 * @date 2016-03-17
 */
public class PyUIDisplayHelper {
...
//    private static int[] sLandscapeRealSizeCache = null;
    ...
    public static int getScreenHeight(Context context) {
        ...
        if(PyUIDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)){
...
//        }
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, PyUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), PyUINotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && PyUIDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * PyUIStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (PyUIDeviceHelper.isHuawei() && !PyUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
                result -= PyUINotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, PyUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), PyUINotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && PyUIDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * PyUIStatusBarHelper.getStatusbarHeight(context);
