 ...
 * @date 2016-03-27
 */
public class PyUIStatusBarHelper {
    ...
    private static boolean supportTranslucent() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
                ...
                && !(PyUIDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT < 26);
    }
    ...
    public static void translucent(Window window, @ColorInt int colorOn5x) {
        ...
        if (PyUINotchHelper.isNotchOfficialSupport()) {
            handleDisplayCutoutMode(window);
        }

        // 小米和魅族4.4 以上版本支持沉浸式
        // 小米 Android 6.0 ，开发版 7.7.13 及以后版本设置黑色字体又需要 clear FLAG_TRANSLUCENT_STATUS, 因此还原为官方模式
        if (PyUIDeviceHelper.isFlymeLowerThan8() || (PyUIDeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
...
//            }
    ...
    public static boolean setStatusBarLightMode(Activity activity) {
        ...
        if (PyUIDeviceHelper.isZTKC2016()) {
            return false;
        }
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        if (PyUIDeviceHelper.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return true;
        }
        return PyUIDeviceHelper.isMIUIV5() || PyUIDeviceHelper.isMIUIV6() ||
                PyUIDeviceHelper.isMIUIV7() || PyUIDeviceHelper.isMIUIV8();
    }
    ...
    public static boolean supportTransclentStatusBar6() {
        return !(PyUIDeviceHelper.isZUKZ1() || PyUIDeviceHelper.isZTKC2016());
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (PyUIDeviceHelper.isTablet(context)
                && sStatusbarHeight > PyUIDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
