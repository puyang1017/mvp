package com.android.puy.puymvpjava.guideview;
...
public class GuideView extends RelativeLayout {
        ...
        if (mParams.mHintView != null) {
            ...
            switch (mParams.mDirection) {
                ...
                case Direction.BOTTOM_CENTER:
                    this.setGravity(Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
                    layoutParams.setMargins(0, 0, 0, mParams.mHintViewMarginBottom);
                    break;
                case Direction.TOP_CENTER:
                    this.setGravity(Gravity.TOP | Gravity.CENTER_HORIZONTAL);
                    layoutParams.setMargins(0, mParams.mHintViewMarginTop, 0, 0);
                    break;
    ...
    public void show() {
        ...
        if (hasShown()) {
            System.out.println("hasShown:" + hasShown());
            return;
        }
        addHintView();
    ...
    public void showOnce() {
        if (mParams.mTargetView != null) {
            getContext().getSharedPreferences(TAG, Context.MODE_PRIVATE).edit().putBoolean(generateUniqId(mParams.mTargetView), true).commit();
        }
    }
    ...
    private boolean hasShown() {
        if (mParams.mTargetView == null)
            return true;
        return getContext().getSharedPreferences(TAG, Context.MODE_PRIVATE).getBoolean(generateUniqId(mParams.mTargetView), false);
    }
    ...
    public interface Direction {
        ...
        int BOTTOM_CENTER = 51;//底部剧中
        int TOP_CENTER = 52;//顶部剧中
        int TOP_RIGHT = 53;//顶部剧中
    }
}